package com.deeplake.noteblockne.item.consumables;

import com.deeplake.noteblockne.entity.creatures.EntityModUnit;
import com.deeplake.noteblockne.item.ItemBase;
import com.deeplake.noteblockne.util.CommonFunctions;
import com.deeplake.noteblockne.util.EntityUtil;
import com.deeplake.noteblockne.util.NBTStrDef.IDLNBTUtil;
...
import static com.deeplake.noteblockne.util.NBTStrDef.IDLNBTDef.MARK_TOTAL_COUNT;
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer playerIn, EntityLivingBase target, EnumHand handIn) {
        if (target instanceof EntityModUnit)
        {
            ...
            if (legalTarget.is_mechanic)
            {
                ...
                {
                    CommonFunctions.SafeSendMsgToPlayer(playerIn, "noteblockne.msg.upgrade_module.req_lv", String.valueOf(requireLv));
                    return false;
                }
                ...
                if (!playerIn.world.isRemote)
                {
                    ...
                    if (success)
                    {
                        ...
                        CommonFunctions.SafeSendMsgToPlayer(playerIn, "noteblockne.msg.upgrade_module.ok", valueBefore, valueAfter);
